...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // Ch7 specific local storage key
    const savedTab = localStorage.getItem('ch7_active_tab') || 'lecture';
    ...
    const switchTab = (targetId) => {
        ...
        if(btn && content) {
            ...
            // Accelerated 'y' axis drop for hardware performance
            ...
            localStorage.setItem('ch7_active_tab', targetId);

            // Initialize specific tab logic securely
            if (targetId === 'mcq' && !window.mcqLoaded_ch7) {
                if(typeof loadMCQ === 'function') loadMCQ();
                window.mcqLoaded_ch7 = true;
            }
            if (targetId === 'cq' && !window.cqLoaded_ch7) {
                if(typeof loadCQ === 'function') loadCQ();
                window.cqLoaded_ch7 = true;
            }
        }
    };
    ...
    // Resume exactly where the user left off in Ch7
    let currentPageIndex = parseInt(localStorage.getItem('ch7_lecture_page') || '0', 10);
    ...
    // Silent Background Pre-fetcher targeting ch7 data
    const preloadLectures = () => {
        lecturePages.forEach((page, index) => {
            if(index !== currentPageIndex) { 
                fetch(`../topic-data/ch7/${page.file}`)
    ...
    // Fetch the lecture JSON metadata for Ch7
    fetch('../topic-data/ch7/lecture.json')
        ...
        .catch(err => {
            viewer.innerHTML = `<div style="color:var(--ch-accent); text-align:center; padding: 40px;" class="bn-text arch-card">ডাটাবেস কানেকশন এরর!</div>`;
        });
    ...
    function loadLecturePage(index, shouldScroll = false) {
        ...
        localStorage.setItem('ch7_lecture_page', currentPageIndex); 
        ...
        fetch(`../topic-data/ch7/${pageData.file}`)
            ...
            .catch(err => {
                viewer.innerHTML = `<div style="color:var(--text-muted); text-align:center; padding: 60px 20px; background: var(--bg-main); border-radius: 20px 20px 8px 8px; border: 1px dashed var(--border-color);">
    ...
    // Hardware accelerated hero animations (no scale, only translate/opacity)
    if (typeof gsap !== 'undefined') {
