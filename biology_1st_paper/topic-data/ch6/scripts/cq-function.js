// topic-data/ch6/scripts/cq-function.js
...
window.loadCQ = function() {
    ...
    Promise.all([
        fetch('../topic-data/ch6/cq/ka.json').then(r => r.json()),
        fetch('../topic-data/ch6/cq/kha.json').then(r => r.json()),
        fetch('../topic-data/ch6/cq/ga-gha.json').then(r => r.json())
    ]).then(([ka, kha, gagha]) => {
        cachedData['ka'] = ka; cachedData['kha'] = kha; cachedData['ga-gha'] = gagha;
        populateFilters(cachedData);
        renderCQs();
    }).catch(err => { container.innerHTML = '<div style="color:red; text-align:center;" class="bn-text">ডেটা লোড করতে ব্যর্থ হয়েছে।</div>'; });
    ...
    const createAccordion = (q, a, typeLabel) => {
        ...
        // Fixed: Wrapper around icon so lucide doesn't break our querySelector later
