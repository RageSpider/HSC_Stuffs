...
document.addEventListener("DOMContentLoaded", () => {
    ...
    let allEntries = [];
    let currentEntries = [];
    let favorites = JSON.parse(localStorage.getItem('diary_favorites')) || [];
    ...
    // Fetch
    fetch('/topic-data/diary/diary.json')
        .then(res => res.json())
        .then(data => {
            allEntries = data;
    ...
    function renderData() {
        ...
        if (currentEntries.length === 0) {
            container.innerHTML = `
                <div class="empty-state bn-text">
                    ...
                    <h3>কোনো দিনলিপি পাওয়া যায়নি</h3>
        ...
        currentEntries.forEach((item) => {
            ...
            html += `
                <div class="diary-item" data-id="${item.id}">
                    <div class="diary-header-clickable">
                        <div class="diary-title-area">
                            <h3 class="diary-question bn-text">${item.question}</h3>
                            <div class="diary-meta bn-text">${boardsHtml}</div>
                        </div>
                    ...
                    <div class="diary-answer-wrapper">
                        <div class="diary-answer bn-text">${item.answer}</div>
                    </div>
                </div>
            `;
        });
        ...
        document.querySelectorAll('.diary-item').forEach(item => {
            const header = item.querySelector('.diary-header-clickable');
            const favBtn = item.querySelector('.btn-fav');
            const answerWrapper = item.querySelector('.diary-answer-wrapper');
    ...
    function toggleFavorite(id) {
        ...
        localStorage.setItem('diary_favorites', JSON.stringify(favorites));
    }
});