...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // Ch5 specific local storage key
    const savedTab = localStorage.getItem('ch5_active_tab') || 'lecture';
    ...
    const switchTab = (targetId) => {
        ...
        if(btn && content) {
            ...
            localStorage.setItem('ch5_active_tab', targetId);

            // Initialize specific tab logic securely
            if (targetId === 'mcq' && !window.mcqLoaded_ch5) {
                if(typeof loadMCQ === 'function') loadMCQ();
                window.mcqLoaded_ch5 = true;
            }
            if (targetId === 'cq' && !window.cqLoaded_ch5) {
                if(typeof loadCQ === 'function') loadCQ();
                window.cqLoaded_ch5 = true;
            }
        }
    };
    ...
    // Resume exactly where the user left off in Ch5
    let currentPageIndex = parseInt(localStorage.getItem('ch5_lecture_page') || '0', 10);
    ...
    // Silent Background Pre-fetcher targeting ch5 data
    const preloadLectures = () => {
        lecturePages.forEach((page, index) => {
            if(index !== currentPageIndex) { 
                fetch(`../topic-data/ch5/${page.file}`)
    ...
    // Fetch the lecture JSON metadata for Ch5
    fetch('../topic-data/ch5/lecture.json')
        ...
        .catch(err => {
            viewer.innerHTML = `<div style="color:var(--ch-accent); text-align:center; padding: 40px; border: 2px dashed var(--ch-accent); border-radius: 16px;" class="bn-text">লেকচার ডেটা লোড করতে সমস্যা হয়েছে!</div>`;
        });
    ...
    function loadLecturePage(index, shouldScroll = false) {
        ...
        localStorage.setItem('ch5_lecture_page', currentPageIndex); 
        ...
        viewer.innerHTML = `<div class="loader bn-text"><i data-lucide="loader-2" class="spin"></i> স্পোর লোড হচ্ছে...</div>`;
        if (typeof lucide !== 'undefined') lucide.createIcons();

        fetch(`../topic-data/ch5/${pageData.file}`)
            ...
            .catch(err => {
                viewer.innerHTML = `<div style="color:var(--text-muted); text-align:center; padding: 60px 20px; border: 2px dashed var(--border-color); border-radius: 16px;">
    ...
    if (typeof gsap !== 'undefined') {
        ...
        tl.fromTo(".ch-badge", { scale: 0.9, opacity: 0 }, { scale: 1, opacity: 1, duration: 0.8, ease: "power2.out", delay: 0.1 })
