...
window.loadCQ = function() {
    ...
    Promise.all([
        fetch('../topic-data/ch8/cq/ka.json').then(r => r.json()),
        fetch('../topic-data/ch8/cq/kha.json').then(r => r.json()),
        fetch('../topic-data/ch8/cq/ga-gha.json').then(r => r.json())
    ]).then(([ka, kha, gagha]) => {
    ...
    const createAccordion = (q, a, typeLabel) => {
        ...
        item.className = 'accordion-item cell-wall-border bn-text';
    ...
    function renderCQs() {
        ...
        if (data.length === 0) {
            container.innerHTML = '<div style="text-align:center; color:var(--text-muted); padding: 30px;" class="bn-text cell-wall-border">কোনো অ্যানালাইসিস পাওয়া যায়নি।</div>';
            return;
        }

        data.forEach((item, index) => {
            const block = document.createElement('div');
            block.className = 'academic-card cell-layer bn-text';
            block.id = item.id;

            let badgeHtml = '';
            if(item.board) badgeHtml += `<span class="card-badge-item cell-wall-border bn-text">${item.board}</span>`;
            if(item.type) badgeHtml += `<span class="card-badge-item cell-wall-border bn-text" style="color:var(--ch-accent);">${item.type}</span>`;
            ...
            if (currentType === 'ga-gha') {
                ...
                stemDiv.style.marginBottom = '20px'; stemDiv.style.color = 'var(--text-dark)'; stemDiv.className = 'bn-text cell-wall-border';
