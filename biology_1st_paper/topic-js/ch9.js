...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // Ch9 specific local storage key
    const savedTab = localStorage.getItem('ch9_active_tab') || 'lecture';
    ...
    const switchTab = (targetId) => {
        ...
        if(btn && content) {
            ...
            // "Flow" left-to-right entrance animation
            if (typeof gsap !== 'undefined') {
                gsap.fromTo(content, {opacity: 0, x: -15}, {opacity: 1, x: 0, duration: 0.4, ease: "power2.out"});
            } else {
                setTimeout(() => { content.classList.add('active'); }, 50);
            }
            localStorage.setItem('ch9_active_tab', targetId);

            // Initialize specific tab logic securely
            if (targetId === 'mcq' && !window.mcqLoaded_ch9) {
                if(typeof loadMCQ === 'function') loadMCQ();
                window.mcqLoaded_ch9 = true;
            }
            if (targetId === 'cq' && !window.cqLoaded_ch9) {
                if(typeof loadCQ === 'function') loadCQ();
                window.cqLoaded_ch9 = true;
            }
        }
    };
    ...
    // Resume exactly where the user left off in Ch9
    let currentPageIndex = parseInt(localStorage.getItem('ch9_lecture_page') || '0', 10);
    ...
    // Silent Background Pre-fetcher targeting ch9 data
    const preloadLectures = () => {
        lecturePages.forEach((page, index) => {
            if(index !== currentPageIndex) { 
                fetch(`../topic-data/ch9/${page.file}`)
    ...
    // Fetch the lecture JSON metadata for Ch9
    fetch('../topic-data/ch9/lecture.json')
        ...
        .catch(err => {
            viewer.innerHTML = `<div style="color:var(--ch-accent); text-align:center; padding: 40px;" class="bn-text flow-card">সিস্টেম কানেকশন এরর!</div>`;
        });
    ...
    function renderContent(htmlContent) {
        ...
        // Smooth hardware accelerated fade
        if (typeof gsap !== 'undefined') gsap.fromTo(viewer, {opacity: 0}, {opacity: 1, duration: 0.3});
    }

    function loadLecturePage(index, shouldScroll = false) {
        if (index < 0 || index >= lecturePages.length) return;
        currentPageIndex = index;
        localStorage.setItem('ch9_lecture_page', currentPageIndex); 
        ...
        viewer.innerHTML = `<div class="loader bn-text"><i data-lucide="loader-2" class="spin"></i> এটিপি জেনারেট হচ্ছে...</div>`;
        if (typeof lucide !== 'undefined') lucide.createIcons();

        fetch(`../topic-data/ch9/${pageData.file}`)
            ...
            .catch(err => {
                viewer.innerHTML = `<div style="color:var(--text-muted); text-align:center; padding: 60px 20px; background: var(--bg-main); border-radius: 0 40px 40px 0; border: 1px dashed var(--border-color); border-left: 4px solid var(--border-color);">
                    ...
                    <p class="bn-text">এই ধাপের ডাটা (${pageData.file}) সিস্টেমে পাওয়া যায়নি।</p>
                </div>`;
                if (typeof lucide !== 'undefined') lucide.createIcons();
            });
    }
    ...
    // Hardware accelerated hero animations (Flowing from left to right)
    if (typeof gsap !== 'undefined') {
        const tl = gsap.timeline();
        tl.fromTo(".ch-badge", { x: -25, opacity: 0 }, { x: 0, opacity: 1, duration: 0.6, ease: "power2.out", delay: 0.1 })
          .fromTo(".ch-title", { x: -20, opacity: 0 }, { x: 0, opacity: 1, duration: 0.7, ease: "power2.out" }, "-=0.4")
