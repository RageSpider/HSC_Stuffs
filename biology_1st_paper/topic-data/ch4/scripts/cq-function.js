...
window.loadCQ = function() {
    ...
    Promise.all([
        fetch('../topic-data/ch4/cq/ka.json').then(r => r.json()),
        fetch('../topic-data/ch4/cq/kha.json').then(r => r.json()),
        fetch('../topic-data/ch4/cq/ga-gha.json').then(r => r.json())
    ]).then(([ka, kha, gagha]) => {
        cachedData['ka'] = ka; cachedData['kha'] = kha; cachedData['ga-gha'] = gagha;
        populateFilters(cachedData);
        renderCQs();
    }).catch(err => { container.innerHTML = '<div style="color:var(--ch-accent); text-align:center; padding: 20px; background:var(--bg-main);" class="bn-text">ডেটা লোড করতে ব্যর্থ হয়েছে।</div>'; });
    ...
    function renderCQs() {
        ...
        data.forEach((item, index) => {
            ...
            if(item.type) badgeHtml += `<span class="card-badge-item bn-text" style="color:var(--ch-accent); background:rgba(106, 27, 154, 0.1);">${item.type}</span>`;
            ...
            if (currentType === 'ga-gha') {
                ...
                stemDiv.style.clipPath = 'polygon(10px 0, calc(100% - 10px) 0, 100% 10px, 100% calc(100% - 10px), calc(100% - 10px) 100%, 10px 100%, 0 calc(100% - 10px), 0 10px)';
            ...
            shadowWrapper.appendChild(block);
