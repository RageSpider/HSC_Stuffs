...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // Ch2 specific local storage key
    const savedTab = localStorage.getItem('ch2_active_tab') || 'lecture';
    ...
    const switchTab = (targetId) => {
        ...
        if(btn && content) {
            ...
            // Hard snap transform instead of soft float for brutalist design
            if (typeof gsap !== 'undefined') {
                gsap.fromTo(content, {opacity: 0, scale: 0.98}, {opacity: 1, scale: 1, duration: 0.3, ease: "power1.out"});
            } else {
                setTimeout(() => { content.classList.add('active'); }, 50);
            }
            localStorage.setItem('ch2_active_tab', targetId);

            // Initialize specific tab logic securely
            if (targetId === 'mcq' && !window.mcqLoaded_ch2) {
                if(typeof loadMCQ === 'function') loadMCQ();
                window.mcqLoaded_ch2 = true;
            }
            if (targetId === 'cq' && !window.cqLoaded_ch2) {
                if(typeof loadCQ === 'function') loadCQ();
                window.cqLoaded_ch2 = true;
            }
        }
    };
    ...
    // Resume exactly where the user left off in Ch2
    let currentPageIndex = parseInt(localStorage.getItem('ch2_lecture_page') || '0', 10);
    ...
    // Silent Background Pre-fetcher targeting ch2 data
    const preloadLectures = () => {
        lecturePages.forEach((page, index) => {
            if(index !== currentPageIndex) { 
                fetch(`../topic-data/ch2/${page.file}`)
    ...
    // Fetch the lecture JSON metadata for Ch2
    fetch('../topic-data/ch2/lecture.json')
        ...
        .catch(err => {
            viewer.innerHTML = `<div style="color:var(--ch-accent); text-align:center; padding: 40px; border: 2px solid var(--ch-accent);" class="bn-text">লেকচার ডেটা লোড করতে সমস্যা হয়েছে!</div>`;
        });
    ...
    function renderContent(htmlContent) {
        ...
        if (typeof gsap !== 'undefined') gsap.fromTo(viewer, {opacity: 0}, {opacity: 1, duration: 0.2});
    }

    function loadLecturePage(index, shouldScroll = false) {
        if (index < 0 || index >= lecturePages.length) return;
        currentPageIndex = index;
        localStorage.setItem('ch2_lecture_page', currentPageIndex); 
        ...
        fetch(`../topic-data/ch2/${pageData.file}`)
            ...
            .catch(err => {
                viewer.innerHTML = `<div style="color:var(--text-muted); text-align:center; padding: 60px 20px; border: 2px dashed var(--text-muted);">
    ...
    if (typeof gsap !== 'undefined') {
        ...
        tl.fromTo(".ch-badge", { x: -20, opacity: 0 }, { x: 0, opacity: 1, duration: 0.6, ease: "power3.out", delay: 0.1 })
          .fromTo(".ch-title", { x: -20, opacity: 0 }, { x: 0, opacity: 1, duration: 0.6, ease: "power3.out" }, "-=0.4")
          .fromTo(".ch-subtitle", { y: 20, opacity: 0 }, { y: 0, opacity: 1, duration: 0.6, ease: "power3.out" }, "-=0.4");
    }
});