// topic-data/ch5/scripts/mcq-function.js
...
window.loadMCQ = function() {
    ...
    // TARGETING CH5 DIRECTORY
    fetch('../topic-data/ch5/mcq.json')
        .then(res => res.json())
        .then(data => { 
            allMcqData = data; 
        ...
        .catch(err => { container.innerHTML = '<div style="color:var(--ch-accent); text-align:center; padding: 20px; border: 2px dashed var(--ch-accent); border-radius:12px;" class="bn-text">ডেটা লোড করতে সমস্যা হয়েছে!</div>'; });
    ...
    function renderMCQs() {
        ...
        if (filtered.length === 0) {
            container.innerHTML = '<div style="text-align:center; color:var(--text-muted); padding: 30px; border: 2px dashed var(--border-color); border-radius:12px;" class="bn-text">কোনো প্রশ্ন পাওয়া যায়নি।</div>';
            return;
        }
        ...
        filtered.forEach((mcq, index) => {
            ...
            if(mcq.type) badgeHtml += `<span class="card-badge-item bn-text" style="color:var(--ch-accent); border-color:var(--ch-accent);">${mcq.type}</span>`;
            ...
            if (!isStudyMode) {
                ...
                btn.addEventListener('click', () => {
                    ...
                    if (selectedIndex === mcq.correctIndex) {
                        ...
                        labels[selectedIndex].style.borderStyle = "dashed";
                        labels[selectedIndex].style.borderColor = "var(--ch-primary)";
                    } else {
                        ...
                        labels[mcq.correctIndex].style.borderStyle = "dashed";
                        labels[mcq.correctIndex].style.borderColor = "var(--ch-primary)";
                    }
