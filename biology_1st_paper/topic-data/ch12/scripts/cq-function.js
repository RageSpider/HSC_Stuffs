...
window.loadCQ = function() {
    ...
    Promise.all([
        fetch('../topic-data/ch2/cq/ka.json').then(r => r.json()),
        fetch('../topic-data/ch2/cq/kha.json').then(r => r.json()),
        fetch('../topic-data/ch2/cq/ga-gha.json').then(r => r.json())
    ]).then(([ka, kha, gagha]) => {
        cachedData['ka'] = ka; cachedData['kha'] = kha; cachedData['ga-gha'] = gagha;
        populateFilters(cachedData);
        renderCQs();
    }).catch(err => { container.innerHTML = '<div style="color:red; text-align:center; padding: 20px; border: 2px solid red;" class="bn-text">ডেটা লোড করতে ব্যর্থ হয়েছে।</div>'; });
    ...
    function renderCQs() {
        ...
        data.forEach((item, index) => {
            ...
            if (currentType === 'ga-gha') {
                ...
                stemDiv.style.padding = '20px'; stemDiv.style.background = 'var(--bg-main)'; stemDiv.style.border = '2px solid var(--border-color)'; stemDiv.style.borderRadius = '0'; stemDiv.style.marginBottom = '20px'; stemDiv.style.color = 'var(--text-dark)'; stemDiv.className = 'bn-text';
