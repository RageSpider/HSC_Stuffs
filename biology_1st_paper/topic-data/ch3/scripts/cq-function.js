// topic-data/ch3/scripts/cq-function.js
...
window.loadCQ = function() {
    ...
    // TARGETING CH3 DIRECTORY
    Promise.all([
        fetch('../topic-data/ch3/cq/ka.json').then(r => r.json()),
        fetch('../topic-data/ch3/cq/kha.json').then(r => r.json()),
        fetch('../topic-data/ch3/cq/ga-gha.json').then(r => r.json())
    ]).then(([ka, kha, gagha]) => {
        cachedData['ka'] = ka; cachedData['kha'] = kha; cachedData['ga-gha'] = gagha;
        populateFilters(cachedData);
        renderCQs();
    }).catch(err => { container.innerHTML = '<div style="color:var(--ch-accent); text-align:center; padding: 20px; border: 1px dashed var(--ch-accent); border-radius: 20px 0 20px 0;" class="bn-text">ডেটা লোড করতে ব্যর্থ হয়েছে।</div>'; });
    ...
    function renderCQs() {
        ...
        if (data.length === 0) {
            container.innerHTML = '<div style="text-align:center; color:var(--text-muted); padding: 30px; border: 1px dashed var(--border-color); border-radius: 20px 0 20px 0;" class="bn-text">কোনো প্রশ্ন পাওয়া যায়নি।</div>';
            return;
        }
        ...
        data.forEach((item, index) => {
            ...
            if(item.type) badgeHtml += `<span class="card-badge-item bn-text" style="color:var(--ch-accent); border-color:var(--ch-accent);">${item.type}</span>`;
            ...
            if (currentType === 'ga-gha') {
                ...
                stemDiv.style.padding = '20px'; stemDiv.style.background = 'var(--bg-main)'; stemDiv.style.border = '1px dashed var(--border-color)'; stemDiv.style.borderRadius = '15px 0 15px 0'; stemDiv.style.marginBottom = '20px'; stemDiv.style.color = 'var(--text-dark)'; stemDiv.className = 'bn-text';
