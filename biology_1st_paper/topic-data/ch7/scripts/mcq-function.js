// topic-data/ch7/scripts/mcq-function.js
...
window.loadMCQ = function() {
    ...
    // TARGETING CH7 DIRECTORY
    fetch('../topic-data/ch7/mcq.json')
        .then(res => res.json())
        .then(data => { 
            allMcqData = data; 
        ...
        .catch(err => { container.innerHTML = '<div style="color:var(--ch-accent); text-align:center; padding: 20px;" class="bn-text arch-card">ডাটাবেস এরর!</div>'; });
    ...
    function renderMCQs() {
        ...
        if (filtered.length === 0) {
            container.innerHTML = '<div style="text-align:center; color:var(--text-muted); padding: 30px;" class="bn-text arch-card">কোনো ডাটা পাওয়া যায়নি।</div>';
            return;
        }
        ...
        filtered.forEach((mcq, index) => {
            ...
            card.className = 'academic-card arch-card bn-text';
            ...
            if(mcq.type) badgeHtml += `<span class="card-badge-item bn-text" style="color:var(--ch-accent); background:rgba(233, 30, 99, 0.05);">${mcq.type}</span>`;

            let optionsHtml = '';
            mcq.rawOptions.forEach((opt, oIndex) => {
                optionsHtml += `
                    <label class="mcq-option-label bn-text">
                        <input type="radio" name="q${mcq.id}" value="${oIndex}" style="margin-right: 12px; accent-color: var(--ch-accent);">
                        ${opt}
                    </label>
                `;
            });
