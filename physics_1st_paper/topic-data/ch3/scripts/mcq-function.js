    ...
    const renderMCQs = async () => {
        ...
        try {
            if (allMcqData.length === 0) {
                const response = await fetch('../topic-data/ch3/mcq.json');
            ...
            displayData.forEach((item, index) => {
                ...
                if (item.type === 'dynamic' && window.Ch3Generators && window.Ch3Generators[item.generator]) {
                    rawData = window.Ch3Generators[item.generator]();
