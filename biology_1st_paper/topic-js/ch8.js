...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // Ch8 specific local storage key
    const savedTab = localStorage.getItem('ch8_active_tab') || 'lecture';
    ...
    const switchTab = (targetId) => {
        ...
        if(btn && content) {
            ...
            // PERFORMANCE FIX: Replaced expensive scaleY animation with hardware-accelerated translateY (y)
            ...
            localStorage.setItem('ch8_active_tab', targetId);

            // Initialize specific tab logic securely
            if (targetId === 'mcq' && !window.mcqLoaded_ch8) {
                if(typeof loadMCQ === 'function') loadMCQ();
                window.mcqLoaded_ch8 = true;
            }
            if (targetId === 'cq' && !window.cqLoaded_ch8) {
                if(typeof loadCQ === 'function') loadCQ();
                window.cqLoaded_ch8 = true;
            }
        }
    };
    ...
    // Resume exactly where the user left off in Ch8
    let currentPageIndex = parseInt(localStorage.getItem('ch8_lecture_page') || '0', 10);
    ...
    // Silent Background Pre-fetcher targeting ch8 data
    const preloadLectures = () => {
        lecturePages.forEach((page, index) => {
            if(index !== currentPageIndex) { 
                fetch(`../topic-data/ch8/${page.file}`)
    ...
    // Fetch the lecture JSON metadata for Ch8
    fetch('../topic-data/ch8/lecture.json')
        ...
        .catch(err => {
            viewer.innerHTML = `<div style="color:var(--ch-accent); text-align:center; padding: 40px;" class="bn-text cell-wall-border">টিস্যু ডাটাবেস এরর!</div>`;
        });
    ...
    function renderContent(htmlContent) {
        ...
        // PERFORMANCE FIX: Simple opacity fade instead of scale transformations
    ...
    function loadLecturePage(index, shouldScroll = false) {
        ...
        localStorage.setItem('ch8_lecture_page', currentPageIndex); 
        ...
        viewer.innerHTML = `<div class="loader bn-text"><i data-lucide="loader-2" class="spin"></i> কোষ বিভাজন চলছে...</div>`;
        if (typeof lucide !== 'undefined') lucide.createIcons();

        fetch(`../topic-data/ch8/${pageData.file}`)
            ...
            .catch(err => {
                viewer.innerHTML = `<div style="color:var(--text-muted); text-align:center; padding: 60px 20px;" class="cell-wall-border">
                    ...
                    <p class="bn-text">এই পৃষ্ঠার কনটেন্ট (${pageData.file}) সিস্টেমে পাওয়া যায়নি।</p>
                </div>`;
                if (typeof lucide !== 'undefined') lucide.createIcons();
            });
    }
    ...
    // PERFORMANCE FIX: Removed scaleX from initial hero animations, replaced with hardware accelerated translateY (y)
    if (typeof gsap !== 'undefined') {
        const tl = gsap.timeline();
        tl.fromTo(".ch-badge", { y: 20, opacity: 0 }, { y: 0, opacity: 1, duration: 0.6, ease: "power2.out", delay: 0.1 })
          .fromTo(".ch-title", { y: 20, opacity: 0 }, { y: 0, opacity: 1, duration: 0.7, ease: "power2.out" }, "-=0.4")
