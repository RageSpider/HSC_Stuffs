...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // Ch1 specific local storage key
    const savedTab = localStorage.getItem('ch1_active_tab') || 'lecture';
    ...
    const switchTab = (targetId) => {
        ...
        if(btn && content) {
            ...
            if (targetId === 'mcq' && !window.mcqLoaded_ch1) {
                if(typeof loadMCQ === 'function') loadMCQ();
                window.mcqLoaded_ch1 = true;
            }
            if (targetId === 'cq' && !window.cqLoaded_ch1) {
                if(typeof loadCQ === 'function') loadCQ();
                window.cqLoaded_ch1 = true;
            }
        }
    };
    ...
    // Resume exactly where the user left off in Ch1
    let currentPageIndex = parseInt(localStorage.getItem('ch1_lecture_page') || '0', 10);
    ...
    // Silent Background Pre-fetcher targeting ch1 data
    const preloadLectures = () => {
        lecturePages.forEach((page, index) => {
            if(index !== currentPageIndex) { 
                fetch(`../topic-data/ch1/${page.file}`)
    ...
    // Fetch the lecture JSON metadata for Ch1
    fetch('../topic-data/ch1/lecture.json')
    ...
    function loadLecturePage(index, shouldScroll = false) {
        ...
        localStorage.setItem('ch1_lecture_page', currentPageIndex); 
        ...
        fetch(`../topic-data/ch1/${pageData.file}`)
