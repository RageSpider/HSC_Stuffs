// topic-data/ch9/scripts/cq-function.js
...
window.loadCQ = function() {
    ...
    Promise.all([
        fetch('../topic-data/ch9/cq/ka.json').then(r => r.json()),
        fetch('../topic-data/ch9/cq/kha.json').then(r => r.json()),
        fetch('../topic-data/ch9/cq/ga-gha.json').then(r => r.json())
    ]).then(([ka, kha, gagha]) => {
    ...
    function renderCQs() {
        ...
        data.forEach((item, index) => {
            ...
            block.className = 'academic-card flow-card bn-text';
            block.id = item.id;

            let badgeHtml = '';
            if(item.board) badgeHtml += `<span class="card-badge-item bn-text">${item.board}</span>`;
            if(item.type) badgeHtml += `<span class="card-badge-item bn-text" style="color:var(--ch-accent); border-left-color:var(--ch-accent); background:transparent;">${item.type}</span>`;
            ...
            if (currentType === 'ga-gha') {
                ...
                stemDiv.style.border = '1px solid var(--border-color)'; stemDiv.style.borderLeft = '4px solid var(--border-color)';
                stemDiv.style.borderRadius = '4px 20px 20px 4px'; 
