...
window.loadMCQ = function() {
    ...
    // TARGETING CH4 DIRECTORY
    fetch('../topic-data/ch4/mcq.json')
        .then(res => res.json())
        .then(data => { 
            allMcqData = data; 
        ...
        .catch(err => { container.innerHTML = '<div style="color:var(--ch-accent); text-align:center; padding: 20px; background:var(--bg-main);" class="bn-text">ডেটা লোড করতে সমস্যা হয়েছে!</div>'; });
    ...
    function renderMCQs() {
        ...
        if (filtered.length === 0) {
            container.innerHTML = '<div style="text-align:center; color:var(--text-muted); padding: 30px; background:var(--bg-main);" class="bn-text">কোনো প্রশ্ন পাওয়া যায়নি।</div>';
            return;
        }
        ...
        filtered.forEach((mcq, index) => {
            // WRAPPING IN SHADOW CONTAINER FOR CLIP-PATH
            const shadowWrapper = document.createElement('div');
            shadowWrapper.className = 'card-shadow-wrapper';
            ...
            if(mcq.type) badgeHtml += `<span class="card-badge-item bn-text" style="color:var(--ch-accent); background:rgba(106, 27, 154, 0.1);">${mcq.type}</span>`;
            ...
            card.innerHTML = `
                ...
                <div class="feedback-box bn-text" style="display: ${isStudyMode ? 'block' : 'none'};">
                    <strong>সঠিক উত্তর:</strong> ${mcq.rawOptions[mcq.correctIndex]}<br>
            ...
            if (!isStudyMode) {
                ...
                btn.addEventListener('click', () => {
                    ...
                    if (selectedIndex === mcq.correctIndex) {
                        ...
                        labels[selectedIndex].style.borderLeft = "4px solid var(--ch-primary)";
                    } else {
                        labels[selectedIndex].classList.add('wrong');
                        labels[selectedIndex].style.background = "rgba(255,0,0,0.05)";
                        labels[selectedIndex].style.borderLeft = "4px solid red";
                        
                        labels[mcq.correctIndex].classList.add('correct');
                        labels[mcq.correctIndex].style.background = "var(--ch-bg-soft)";
                        labels[mcq.correctIndex].style.borderLeft = "4px solid var(--ch-primary)";
                    }
            ...
            shadowWrapper.appendChild(card);
