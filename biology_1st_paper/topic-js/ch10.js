...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // Ch10 specific local storage key
    const savedTab = localStorage.getItem('ch10_active_tab') || 'lecture';
    ...
    const switchTab = (targetId) => {
        ...
        if(btn && content) {
            ...
            // "Blooming" growth entrance from bottom-left corner
            if (typeof gsap !== 'undefined') {
                gsap.fromTo(content, {opacity: 0, scale: 0.95, rotation: -1, transformOrigin: "bottom left"}, {opacity: 1, scale: 1, rotation: 0, duration: 0.5, ease: "back.out(1.2)"});
            } else {
                setTimeout(() => { content.classList.add('active'); }, 50);
            }
            localStorage.setItem('ch10_active_tab', targetId);

            // Initialize specific tab logic securely
            if (targetId === 'mcq' && !window.mcqLoaded_ch10) {
                if(typeof loadMCQ === 'function') loadMCQ();
                window.mcqLoaded_ch10 = true;
            }
            if (targetId === 'cq' && !window.cqLoaded_ch10) {
                if(typeof loadCQ === 'function') loadCQ();
                window.cqLoaded_ch10 = true;
            }
        }
    };
    ...
    // Resume exactly where the user left off in Ch10
    let currentPageIndex = parseInt(localStorage.getItem('ch10_lecture_page') || '0', 10);
    ...
    // Silent Background Pre-fetcher targeting ch10 data
    const preloadLectures = () => {
        lecturePages.forEach((page, index) => {
            if(index !== currentPageIndex) { 
                fetch(`../topic-data/ch10/${page.file}`)
    ...
    // Fetch the lecture JSON metadata for Ch10
    fetch('../topic-data/ch10/lecture.json')
        ...
        .catch(err => {
            viewer.innerHTML = `<div style="color:var(--ch-accent); text-align:center; padding: 40px;" class="bn-text floral-card">ডাটাবেস কানেকশন এরর!</div>`;
        });
    ...
    function loadLecturePage(index, shouldScroll = false) {
        ...
        localStorage.setItem('ch10_lecture_page', currentPageIndex); 
        ...
        viewer.innerHTML = `<div class="loader bn-text"><i data-lucide="loader-2" class="spin"></i> ডাটা লোড হচ্ছে...</div>`;
        if (typeof lucide !== 'undefined') lucide.createIcons();

        fetch(`../topic-data/ch10/${pageData.file}`)
            ...
            .catch(err => {
                viewer.innerHTML = `<div style="color:var(--text-muted); text-align:center; padding: 60px 20px; background: var(--bg-main);" class="floral-leaf">
