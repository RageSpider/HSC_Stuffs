...
window.loadMCQ = function() {
    ...
    // TARGETING CH8 DIRECTORY
    fetch('../topic-data/ch8/mcq.json')
        .then(res => res.json())
        .then(data => { 
            allMcqData = data; 
        ...
        .catch(err => { container.innerHTML = '<div style="color:var(--ch-accent); text-align:center; padding: 20px;" class="bn-text cell-wall-border">টিস্যু ডাটাবেস এরর!</div>'; });
    ...
    function renderMCQs() {
        ...
        if (filtered.length === 0) {
            container.innerHTML = '<div style="text-align:center; color:var(--text-muted); padding: 30px;" class="bn-text cell-wall-border">কোনো ডাটা পাওয়া যায়নি।</div>';
            return;
        }
        ...
        filtered.forEach((mcq, index) => {
            ...
            card.className = 'academic-card cell-layer bn-text';
            
            const hlQuestion = highlightMatch(mcq.question, searchTerm, banSearchTerm);

            let badgeHtml = '';
            if(mcq.board) badgeHtml += `<span class="card-badge-item cell-wall-border bn-text">${mcq.board}</span>`;
            if(mcq.type) badgeHtml += `<span class="card-badge-item cell-wall-border bn-text" style="color:var(--ch-accent);">${mcq.type}</span>`;

            let optionsHtml = '';
            mcq.rawOptions.forEach((opt, oIndex) => {
                optionsHtml += `
                    <label class="mcq-option-label cell-wall-border bn-text">
            ...
            card.innerHTML = `
                ...
                <div>${optionsHtml}</div>
                <button class="check-ans-btn cell-wall-border bn-text">যাচাই করুন</button>
                <div class="feedback-box cell-wall-border bn-text" style="display: ${isStudyMode ? 'block' : 'none'};">
                    <strong>সঠিক ডাটা:</strong> ${mcq.rawOptions[mcq.correctIndex]}<br>
                    <span style="color:var(--text-muted); font-size:0.95rem; margin-top:8px; display:block;"><strong>ব্যাখ্যা:</strong> ${mcq.explanation}</span>
                </div>
            `;
            ...
            if (!isStudyMode) {
                ...
                btn.addEventListener('click', () => {
                    ...
                    if (selectedIndex === mcq.correctIndex) {
                        ...
                        labels[selectedIndex].style.borderColor = "var(--ch-accent)";
                    } else {
                        labels[selectedIndex].classList.add('wrong');
                        labels[selectedIndex].style.borderColor = "red";
                        
                        labels[mcq.correctIndex].classList.add('correct');
                        labels[mcq.correctIndex].style.background = "var(--ch-bg-soft)";
                        labels[mcq.correctIndex].style.borderColor = "var(--ch-accent)";
                    }
