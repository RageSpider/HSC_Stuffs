...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // Ch4 specific local storage key
    const savedTab = localStorage.getItem('ch4_active_tab') || 'lecture';
    ...
    const switchTab = (targetId) => {
        ...
        if(btn && content) {
            ...
            if (typeof gsap !== 'undefined') {
                gsap.fromTo(content, {opacity: 0, scale: 0.98}, {opacity: 1, scale: 1, duration: 0.4, ease: "power2.out"});
            } else {
            ...
            if (targetId === 'mcq' && !window.mcqLoaded_ch4) {
                if(typeof loadMCQ === 'function') loadMCQ();
                window.mcqLoaded_ch4 = true;
            }
            if (targetId === 'cq' && !window.cqLoaded_ch4) {
                if(typeof loadCQ === 'function') loadCQ();
                window.cqLoaded_ch4 = true;
            }
        }
    };
    ...
    // Resume exactly where the user left off in Ch4
    let currentPageIndex = parseInt(localStorage.getItem('ch4_lecture_page') || '0', 10);
    ...
    // Silent Background Pre-fetcher targeting ch4 data
    const preloadLectures = () => {
        lecturePages.forEach((page, index) => {
            if(index !== currentPageIndex) { 
                fetch(`../topic-data/ch4/${page.file}`)
    ...
    // Fetch the lecture JSON metadata for Ch4
    fetch('../topic-data/ch4/lecture.json')
        ...
        .catch(err => {
            viewer.innerHTML = `<div style="color:var(--ch-accent); text-align:center; padding: 40px; background: var(--bg-main);" class="bn-text">লেকচার ডেটা লোড করতে সমস্যা হয়েছে!</div>`;
        });
    ...
    function loadLecturePage(index, shouldScroll = false) {
        ...
        localStorage.setItem('ch4_lecture_page', currentPageIndex); 
        ...
        fetch(`../topic-data/ch4/${pageData.file}`)
    ...
    if (typeof gsap !== 'undefined') {
        ...
        tl.fromTo(".ch-badge", { scale: 1.1, opacity: 0 }, { scale: 1, opacity: 1, duration: 0.6, ease: "power2.out", delay: 0.1 })
          .fromTo(".ch-title", { opacity: 0 }, { opacity: 1, duration: 0.8, ease: "power2.out" }, "-=0.3")
          .fromTo(".ch-subtitle", { opacity: 0 }, { opacity: 1, duration: 0.8, ease: "power2.out" }, "-=0.5");
    }
});