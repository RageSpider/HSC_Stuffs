// topic-data/ch10/scripts/cq-function.js
...
window.loadCQ = function() {
    ...
    Promise.all([
        fetch('../topic-data/ch10/cq/ka.json').then(r => r.json()),
        fetch('../topic-data/ch10/cq/kha.json').then(r => r.json()),
        fetch('../topic-data/ch10/cq/ga-gha.json').then(r => r.json())
    ]).then(([ka, kha, gagha]) => {
    ...
    function renderCQs() {
        ...
        if (data.length === 0) {
            container.innerHTML = '<div style="text-align:center; color:var(--text-muted); padding: 30px;" class="bn-text floral-card">কোনো অ্যানালাইসিস পাওয়া যায়নি।</div>';
            return;
        }

        data.forEach((item, index) => {
            const block = document.createElement('div');
            block.className = 'academic-card floral-card bn-text';
            block.id = item.id;

            let badgeHtml = '';
            if(item.board) badgeHtml += `<span class="card-badge-item bn-text">${item.board}</span>`;
            if(item.type) badgeHtml += `<span class="card-badge-item bn-text" style="color:var(--ch-accent); background:transparent;">${item.type}</span>`;
            ...
            if (currentType === 'ga-gha') {
                ...
                stemDiv.style.borderRadius = '0 16px 0 16px'; 
