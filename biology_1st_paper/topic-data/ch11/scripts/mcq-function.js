// topic-data/ch12/scripts/mcq-function.js
...
window.loadMCQ = function() {
    ...
    fetch('../topic-data/ch12/mcq.json')
        .then(res => res.json())
        .then(data => { 
            allMcqData = data; 
    ...
    function renderMCQs() {
        ...
        if (filtered.length === 0) {
            container.innerHTML = '<div style="text-align:center; color:var(--text-muted); padding: 30px;" class="bn-text strata-card">কোনো ডাটা পাওয়া যায়নি।</div>';
            return;
        }
        ...
        filtered.forEach((mcq, index) => {
            ...
            card.className = 'academic-card strata-card bn-text';
            
            const hlQuestion = highlightMatch(mcq.question, searchTerm, banSearchTerm);

            let badgeHtml = '';
            if(mcq.board) badgeHtml += `<span class="card-badge-item strata-edge bn-text">${mcq.board}</span>`;
            if(mcq.type) badgeHtml += `<span class="card-badge-item strata-edge bn-text" style="color:var(--ch-accent); background:transparent;">${mcq.type}</span>`;
            ...
            card.innerHTML = `
                ...
                <div>${optionsHtml}</div>
                <button class="check-ans-btn strata-edge bn-text">যাচাই করুন</button>
                <div class="feedback-box strata-edge bn-text" style="display: ${isStudyMode ? 'block' : 'none'};">
