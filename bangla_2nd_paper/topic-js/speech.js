...
document.addEventListener("DOMContentLoaded", () => {
    ...
    let allEntries = [];
    let currentEntries = [];
    let favorites = JSON.parse(localStorage.getItem('speech_favorites')) || [];
    ...
    // Fetch
    fetch('/topic-data/speech/speech.json')
        .then(res => res.json())
        .then(data => {
            allEntries = data;
    ...
    function renderData() {
        ...
        currentEntries.forEach((item) => {
            ...
            html += `
                <div class="speech-item" data-id="${item.id}">
                    <div class="speech-header-clickable">
                        <div class="speech-title-area">
                            <h3 class="speech-question bn-text">${item.question}</h3>
                            <div class="speech-meta bn-text">${boardsHtml}</div>
                        </div>
                    ...
                    <div class="speech-answer-wrapper">
                        <div class="speech-answer bn-text">${item.answer}</div>
                    </div>
                </div>
            `;
        });
        ...
        document.querySelectorAll('.speech-item').forEach(item => {
            const header = item.querySelector('.speech-header-clickable');
            const favBtn = item.querySelector('.btn-fav');
            const answerWrapper = item.querySelector('.speech-answer-wrapper');
    ...
    function toggleFavorite(id) {
        ...
        localStorage.setItem('speech_favorites', JSON.stringify(favorites));
    }
});