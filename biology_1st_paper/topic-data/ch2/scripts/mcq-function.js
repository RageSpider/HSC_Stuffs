// topic-data/ch2/scripts/mcq-function.js
...
window.loadMCQ = function() {
    ...
    // TARGETING CH2 DIRECTORY
    fetch('../topic-data/ch2/mcq.json')
        .then(res => res.json())
        .then(data => { 
            allMcqData = data; 
        ...
        .catch(err => { container.innerHTML = '<div style="color:red; text-align:center; padding: 20px; border: 2px solid red;" class="bn-text">ডেটা লোড করতে সমস্যা হয়েছে!</div>'; });
    ...
    function renderMCQs() {
        ...
        if (filtered.length === 0) {
            container.innerHTML = '<div style="text-align:center; color:var(--text-muted); padding: 30px; border: 2px dashed var(--border-color);" class="bn-text">কোনো প্রশ্ন পাওয়া যায়নি।</div>';
            return;
        }
        ...
        filtered.forEach((mcq, index) => {
            ...
            if (!isStudyMode) {
                ...
                labels[mcq.correctIndex].classList.add('correct');
                labels[mcq.correctIndex].style.borderColor = "var(--ch-primary)";
                labels[mcq.correctIndex].style.borderLeftWidth = "6px";
            }
