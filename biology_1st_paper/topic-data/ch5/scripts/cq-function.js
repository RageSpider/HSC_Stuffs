// topic-data/ch5/scripts/cq-function.js
...
window.loadCQ = function() {
    ...
    Promise.all([
        fetch('../topic-data/ch5/cq/ka.json').then(r => r.json()),
        fetch('../topic-data/ch5/cq/kha.json').then(r => r.json()),
        fetch('../topic-data/ch5/cq/ga-gha.json').then(r => r.json())
    ]).then(([ka, kha, gagha]) => {
        cachedData['ka'] = ka; cachedData['kha'] = kha; cachedData['ga-gha'] = gagha;
        populateFilters(cachedData);
        renderCQs();
    }).catch(err => { container.innerHTML = '<div style="color:var(--ch-accent); text-align:center; padding: 20px; border: 2px dashed var(--ch-accent); border-radius:12px;" class="bn-text">ডেটা লোড করতে ব্যর্থ হয়েছে।</div>'; });
    ...
    function renderCQs() {
        ...
        data.forEach((item, index) => {
            ...
            if (currentType === 'ga-gha') {
                ...
                stemDiv.style.border = '2px dashed var(--border-color)'; stemDiv.style.borderRadius = '16px'; 
