...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // Ch11 specific local storage key
    const savedTab = localStorage.getItem('ch11_active_tab') || 'lecture';
    ...
    const switchTab = (targetId) => {
        ...
        if(btn && content) {
            ...
            localStorage.setItem('ch11_active_tab', targetId);

            // Initialize specific tab logic securely
            if (targetId === 'mcq' && !window.mcqLoaded_ch11) {
                if(typeof loadMCQ === 'function') loadMCQ();
                window.mcqLoaded_ch11 = true;
            }
            if (targetId === 'cq' && !window.cqLoaded_ch11) {
                if(typeof loadCQ === 'function') loadCQ();
                window.cqLoaded_ch11 = true;
            }
        }
    };
    ...
    // Resume exactly where the user left off in Ch11
    let currentPageIndex = parseInt(localStorage.getItem('ch11_lecture_page') || '0', 10);
    ...
    // Silent Background Pre-fetcher targeting ch11 data
    const preloadLectures = () => {
        lecturePages.forEach((page, index) => {
            if(index !== currentPageIndex) { 
                fetch(`../topic-data/ch11/${page.file}`)
    ...
    // Fetch the lecture JSON metadata for Ch11
    fetch('../topic-data/ch11/lecture.json')
        ...
        .catch(err => {
            viewer.innerHTML = `<div style="color:var(--ch-accent); text-align:center; padding: 40px; border: 1px solid var(--glass-border); border-radius: 16px;" class="bn-text glass-panel">ডাটাবেস কানেকশন এরর!</div>`;
        });
    ...
    function loadLecturePage(index, shouldScroll = false) {
        ...
        localStorage.setItem('ch11_lecture_page', currentPageIndex); 
        ...
        viewer.innerHTML = `<div class="loader bn-text"><i data-lucide="loader-2" class="spin"></i> ডিএনএ সিকোয়েন্সিং চলছে...</div>`;
        if (typeof lucide !== 'undefined') lucide.createIcons();

        fetch(`../topic-data/ch11/${pageData.file}`)
            ...
            .catch(err => {
                viewer.innerHTML = `<div style="color:var(--text-muted); text-align:center; padding: 60px 20px; border: 1px solid var(--glass-border); border-radius: 16px;" class="glass-panel">
    ...
    // PERFORMANCE FIX: Removed filter: "blur()" from GSAP animation which drops framerate severely
    if (typeof gsap !== 'undefined') {
        const tl = gsap.timeline();
        tl.fromTo(".ch-badge", { scale: 0.9, opacity: 0, y: 15 }, { scale: 1, opacity: 1, y: 0, duration: 0.8, ease: "power2.out", delay: 0.1 })
          .fromTo(".ch-title", { y: 15, opacity: 0 }, { y: 0, opacity: 1, duration: 0.7, ease: "power2.out" }, "-=0.4")
          .fromTo(".ch-subtitle", { y: 15, opacity: 0 }, { y: 0, opacity: 1, duration: 0.7, ease: "power2.out" }, "-=0.5");
    }
});