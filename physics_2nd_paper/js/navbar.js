...
document.addEventListener("navbarLoaded", () => {
    ...
    const closeMobileMenu = () => {
        ...
        if (hamburgerBtn) {
            const icon = hamburgerBtn.querySelector('i') || hamburgerBtn.querySelector('svg');
    ...
    // Theme Logic for Physics 2nd Paper (Unique Storage Key)
    const themeToggleBtn = document.getElementById('theme-toggle');
    const htmlElement = document.documentElement;
    if (themeToggleBtn) {
        const moonIcon = themeToggleBtn.querySelector('.moon-icon');
        const sunIcon = themeToggleBtn.querySelector('.sun-icon');
        const savedTheme = localStorage.getItem('physics2-theme') || 'light';
        ...
        themeToggleBtn.addEventListener('click', () => {
            ...
            localStorage.setItem('physics2-theme', newTheme);
