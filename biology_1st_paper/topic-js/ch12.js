...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // Ch12 specific local storage key
    const savedTab = localStorage.getItem('ch12_active_tab') || 'lecture';
    ...
    const switchTab = (targetId) => {
        ...
        if(btn && content) {
            ...
            // "Unearthing" upward entrance animation (Hardware Accelerated)
            ...
            localStorage.setItem('ch12_active_tab', targetId);

            // Initialize specific tab logic securely
            if (targetId === 'mcq' && !window.mcqLoaded_ch12) {
                if(typeof loadMCQ === 'function') loadMCQ();
                window.mcqLoaded_ch12 = true;
            }
            if (targetId === 'cq' && !window.cqLoaded_ch12) {
                if(typeof loadCQ === 'function') loadCQ();
                window.cqLoaded_ch12 = true;
            }
        }
    };
    ...
    // Resume exactly where the user left off in Ch12
    let currentPageIndex = parseInt(localStorage.getItem('ch12_lecture_page') || '0', 10);
    ...
    // Silent Background Pre-fetcher targeting ch12 data
    const preloadLectures = () => {
        lecturePages.forEach((page, index) => {
            if(index !== currentPageIndex) { 
                fetch(`../topic-data/ch12/${page.file}`)
    ...
    // Fetch the lecture JSON metadata for Ch12
    fetch('../topic-data/ch12/lecture.json')
        ...
        .catch(err => {
            viewer.innerHTML = `<div style="color:var(--ch-accent); text-align:center; padding: 40px;" class="bn-text strata-card">ইতিহাস ডাটাবেস এরর!</div>`;
        });
    ...
    function loadLecturePage(index, shouldScroll = false) {
        ...
        localStorage.setItem('ch12_lecture_page', currentPageIndex); 
        ...
        viewer.innerHTML = `<div class="loader bn-text"><i data-lucide="loader-2" class="spin"></i> ফসিল রেকর্ড লোড হচ্ছে...</div>`;
        if (typeof lucide !== 'undefined') lucide.createIcons();

        fetch(`../topic-data/ch12/${pageData.file}`)
            ...
            .catch(err => {
                viewer.innerHTML = `<div style="color:var(--text-muted); text-align:center; padding: 60px 20px; background: var(--bg-main);" class="strata-edge">
                    ...
                    <p class="bn-text">এই যুগের রেকর্ড (${pageData.file}) সিস্টেমে পাওয়া যায়নি।</p>
                </div>`;
                if (typeof lucide !== 'undefined') lucide.createIcons();
            });
    }
