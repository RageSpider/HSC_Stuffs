// topic-data/ch10/scripts/mcq-function.js
...
window.loadMCQ = function() {
    ...
    // TARGETING CH10 DIRECTORY
    fetch('../topic-data/ch10/mcq.json')
        .then(res => res.json())
        .then(data => { 
            allMcqData = data; 
        ...
        .catch(err => { container.innerHTML = '<div style="color:var(--ch-accent); text-align:center; padding: 20px;" class="bn-text floral-card">ডাটাবেস এরর!</div>'; });
    ...
    function renderMCQs() {
        ...
        if (filtered.length === 0) {
            container.innerHTML = '<div style="text-align:center; color:var(--text-muted); padding: 30px;" class="bn-text floral-card">কোনো ডাটা পাওয়া যায়নি।</div>';
            return;
        }
        ...
        filtered.forEach((mcq, index) => {
            ...
            card.className = 'academic-card floral-card bn-text';
            ...
            if(mcq.type) badgeHtml += `<span class="card-badge-item bn-text" style="color:var(--ch-accent); background:transparent;">${mcq.type}</span>`;
            ...
            card.innerHTML = `
                ...
                <div>${optionsHtml}</div>
                <button class="check-ans-btn bn-text floral-leaf">যাচাই করুন</button>
